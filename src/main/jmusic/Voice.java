    ...
    public void addNote(Note note) {
        if (channel != 9) {
            ...
            if (layers[0].isEmpty()) {
                ...
                float lastDuration = Float.parseFloat(last.substring(last.indexOf("/") + 1, last.indexOf("a")));

                time = lastTime + (int) (240000 * lastDuration / bpm);
            }

            layers[0].add("@" + time + " &V" + channel + ",L0,I" + instrument + "," + note.getPitch() + "/" + duration + "a" + velocity);
            for (int i = 1; i < 16; i++) {
    ...
    public void addInterval(Interval interval) {
        if (channel != 9) {
            ...
            if (notes.length == 1) {
                ...
                int velocity = notes[0].getVelocity();
                ...
                layers[0].add("@" + time + " &V" + channel + ",L0,I" + instrument + "," + notes[0].getPitch() + "/" + duration + "a" + velocity);
                layers[0].add("@" + time + " &V" + channel + ",L1,I" + instrument + "," + notes[1].getPitch() + "/" + duration + "a" + velocity);
                for (int i = 2; i < 16; i++) {
                    layers[i].add("@" + time + " &V" + channel + ",L" + i + ",I" + instrument + ",R/" + duration);
                }
            }
    ...
    public void addChord(Chord chord) {
        if (channel != 9) {
            ...
            for (int i = 0; i < chordSize; i++) {
                layers[i].add("@" + time + " &V" + channel + ",L" + i + ",I" + instrument + "," + notes[i].getPitch() + "/" + duration + "a" + velocity);
            }
