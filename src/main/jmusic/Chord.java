...
public class Chord {
    ...
    public Chord(int tonic, String chord, float duration) {
        this.tonic = tonic;
        parseChord(chord, 64, duration);
    }

    public Chord(int tonic, String chord, int velocity, float duration) {
        this.tonic = tonic;
        parseChord(chord, velocity, duration);
    }

    private void parseChord(String chord, int velocity, float duration) {
        ...
        for (int i = 0; i < noteValues.size(); i++) {
            notes[i] = new Note(tonic + noteValues.get(i), velocity, duration);
        }
    }
